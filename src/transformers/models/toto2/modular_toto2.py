...
class Toto2Model(Toto2PreTrainedModel):
    ...
    def _build_causal_mask(
        new_len: int, past_len: int, dtype: torch.dtype, device: torch.device
    ) -> torch.Tensor:
        ...
        causal = torch.triu(
            torch.full((new_len, new_len), min_value, dtype=dtype, device=device), diagonal=1
        )
...
class Toto2ForPrediction(Toto2PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        median_idx = (
            self.config.quantiles.index(0.5) if 0.5 in self.config.quantiles else num_q // 2
        )
        ...
        while patches_predicted < num_patches:
            ...
            if past_key_values is not None and patches_predicted > 0:
                ...
                time_ids = all_time_ids[
                    patches_predicted - block_size_patches : patches_predicted + block
                ]
            else:
